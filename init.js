...
bot.action('piar', ctx => {
    ctx.answerCbQuery(); // Ответить на нажатие кнопки без сообщения
    const responseText = `Наша пиар компания поможет вам привлечь внимание к вашему проекту! 💼 \n
За небольшую плату вы получите доступ к закрытому каналу 🔞, где сможете обмениваться фотографиями 📷,
музыкой 🎶, фильмами 🎥 и другими материалами с единомышленниками! 🤝
Важно! Вы также получите возможность рекламировать свои товары и услуги в нашем канале.

Если это предложение заинтересовало вас и вы хотите узнать подробнее о членстве, количество наших подписчиков и стоимости услуг,
нажмите кнопку ниже, чтобы перейти к нашему пиар-менеджеру! 📩`;
    ctx.replyWithMarkdown(responseText, Markup.inlineKeyboard([
        Markup.button.url('Связаться с пиар-менеджером 💬', 'https://t.me/+nndiphj4wElhMWI6')
    ]));
});
...
bot.on('text', (ctx) => {
    if (ctx.session.complainStep === 'waiting_for_complaint') {
        ...
        bot.telegram.sendMessage(6638651166, `Жалоба от пользователя ${ctx.from.username || ctx.from.id}: ${complaintText}`)
            ...
            .catch(err => {
                console.error(err);
                ctx.reply('К сожалению, произошла ошибка при отправке вашей жалобы.');
            });
        ctx.session.complainStep = null;
    }
});
